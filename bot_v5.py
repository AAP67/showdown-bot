"""
Pokemon Showdown Bot — Sprint 5
Play on the public Showdown ladder with your real account.
...
try:
    ...
    HAS_ANTHROPIC = False
    print("  ⚠️  Install with: pip install anthropic")
...
PUBLIC_SERVER = ShowdownServerConfiguration
...
You have access to OPPONENT SET PREDICTIONS showing likely unrevealed moves, items, and abilities. Use these to anticipate threats.
...
class LadderBot(Player):
    """
    Sprint 5: plays on the public Showdown ladder with your real account.
    """
    ...
    def _llm_switch(self, battle, turn, forced=False):
        ...
        state_text = format_battle_state(battle, self.predictor)
        action_type, action, reasoning = self.llm.decide(state_text, [], battle.available_switches)

        if action_type == "switch" and action:
            self.llm_decisions += 1
            label = "forced" if forced else "proactive"
            self.battle_log.log(turn, "switch", f"{action.species} [LLM]", f"🧠 {reasoning} ({label})")
            return action
    ...
    def _heuristic_fallback(self, battle, turn, llm_error=""):
        ...
        if battle.available_moves:
            ...
            self.battle_log.log(turn, "move", f"{best_move.id} ({best_move.base_power}BP{stab}) [FALLBACK]", f"⚙️ ({llm_error})")
            return self.create_order(best_move)

        return self.choose_random_move(battle)

    def _battle_finished_callback(self, battle):
        """Track results after each battle."""
        result = {
            "battle_tag": battle.battle_tag,
            "won": battle.won,
            "opponent": battle.opponent_username,
            "turns": battle.turn,
            "remaining": sum(1 for m in battle.team.values() if not m.fainted),
            "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        }
        self.battle_results.append(result)

        emoji = "✅" if battle.won else "❌"
        print(f"\n  {emoji} {'WIN' if battle.won else 'LOSS'} vs {battle.opponent_username} ({battle.turn} turns, {result['remaining']}/6 remaining)")
...
async def run_ladder(num_battles=1, llm_engine=None):
    ...
    blog = BattleLog(verbose=True)
    bot_config = AccountConfiguration(username, password if password else None)
    bot = LadderBot(
        battle_log=blog,
    ...
    print(f"\n{'='*55}")
    print(f"  📊 LADDER RESULTS — {username}")
    print(f"  Record: {wins}W / {total - wins}L ({wr:.0f}% WR)")
...
async def run_local(num_battles=1, llm_engine=None):
    """Play locally against RandomPlayer."""
    blog = BattleLog(verbose=True)
    bot_config = AccountConfiguration("LadderBot", None)
    opp_config = AccountConfiguration("OppBot", None)
    ...
    print(f"\n{'='*55}")
    print(f"  🎮 LOCAL MODE — {num_battles} battle(s)")
    print(f"{'='*55}\n")
    ...
    print(f"\n  📊 {wins}W / {total - wins}L ({wr:.0f}% WR)")
    print(f"  🧠 LLM: {bot.llm_decisions}/{total_decisions} ({llm_pct:.0f}%)")
    print(f"  📡 Tokens: {bot.llm.total_tokens}")
...
def main():
    ...
    print(f"""
╔═══════════════════════════════════════════════╗
║       🎮  POKEMON SHOWDOWN BOT v5  🎮        ║
║       Sprint 5 — Public Ladder                ║
╚═══════════════════════════════════════════════╝

  Account: {username or '⚠️  NOT SET — add PS_USERNAME to .env'}
  API Key: {'✅ Set' if os.environ.get('ANTHROPIC_API_KEY') else '⚠️  NOT SET'}

Commands:
  ladder [n]     — Play n games on public ladder (default: 1)
  local [n]      — Play n games locally vs RandomPlayer
...
""")
    ...
    while True:
        ...
        if action in ("quit", "exit", "q"):
            ...
            try:
                asyncio.run(run_ladder(n, llm_engine))
            except Exception as e:
                print(f"\n  ❗ Error: {e}\n")
            ...
            n = int(parts[1]) if len(parts) > 1 and parts[1].isdigit() else 1
            try:
                asyncio.run(run_local(n, llm_engine))
            except Exception as e:
                print(f"\n  ❗ Error: {e}\n")
            ...
            print("  Unknown command. Try: ladder, local, stats, quit")
