"""
Pokemon Showdown Bot — Sprint 3
LLM-powered decision making via Claude API, with heuristic fallback.
...
def format_battle_state(battle):
    ...
    if opp_team:
        remaining = [p for p in opp_team.values() if not p.fainted]
        fainted = [p for p in opp_team.values() if p.fainted]
        lines.append(f"\nOPPONENT TEAM: {len(remaining)} remaining, {len(fainted)} fainted")
        for p in remaining:
            if p != opponent:
...
class LLMEngine:
    ...
    def decide(self, battle_state_text, available_moves, available_switches):
        """Ask Claude for a decision. Returns (action_type, action_name, reasoning)."""
        if not self.client:
        ...
        try:
            ...
            # If no match, try without move/switch keyword (assume it's a move name)
            if not action_match:
...
class LLMBot(Player):
    """
    Sprint 3 bot: uses Claude to decide moves, falls back to heuristics on failure.
    """
    ...
    def choose_move(self, battle):
        ...
        if action_type == "move" and action:
            ...
            self.battle_log.log(
                turn, "move",
                f"{action.id} ({action.base_power}BP{stab}) [LLM]",
                f"🧠 {reasoning}"
            )
    ...
    def _llm_switch(self, battle, turn, forced=False):
        """Use LLM for switch decisions too."""
        ...
        if action_type == "switch" and action:
            self.llm_decisions += 1
            label = "forced switch" if forced else "proactive switch"
        ...
        self.battle_log.log(turn, "switch", f"{switch.species} [FALLBACK]", "LLM unavailable for switch")
        return switch

    def _heuristic_fallback(self, battle, turn, llm_error=""):
        """Sprint 2 heuristic as fallback."""
        active = battle.active_pokemon
        opponent = battle.opponent_active_pokemon
...
async def run_battle(bot_class, bot_name, num_battles=1, use_local=True, verbose=True, llm_engine=None):
    ...
    if llm_engine and bot_class == LLMBot:
...
async def benchmark(num_battles=10, use_local=True, llm_engine=None):
    print(f"\n{'='*55}")
    ...
    print(f"  Sprint 3 (LLM Bot):    {s3_wr:.0f}% WR ({s3_wins}/{s3_total})")
...
def main():
    print("""
╔═══════════════════════════════════════════════╗
║       🎮  POKEMON SHOWDOWN BOT v3  🎮        ║
║       Sprint 3 — LLM Decision Layer           ║
╚═══════════════════════════════════════════════╝
""")
    ...
    print("""
Commands:
  start [n]      — Run n battles with LLM Bot (default: 1)
  bench [n]      — Benchmark Sprint2 vs Sprint3 (default: 10 each)
...
""")
    ...
    while True:
        ...
        if action in ("quit", "exit", "q"):
            ...
            try:
                asyncio.run(run_battle(LLMBot, "LLMBot", n, use_local, llm_engine=llm_engine))
            except Exception as e:
                print(f"\n  ❗ Error: {e}\n")
            ...
            if llm_engine:
                ...
                est_cost = llm_engine.total_tokens * 0.000003  # rough sonnet estimate
