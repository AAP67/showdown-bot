...
const MOVES = [
  "earthquake", "scald", "thunderbolt", "moonblast", "flamethrower",
  "icebeam", "closecombat", "shadowball", "dracometeor", "uturn",
  "knockoff", "bravebird", "fleurcannon", "psystrike", "bodyslam",
]

const REASONS = [
  "Super effective STAB move deals maximum damage",
  "Type advantage favors switching to resist incoming attacks",
  "Opponent likely carries Choice Scarf, safe to set up",
  "Predicted Earth Power threatens our position, switching out",
  "STAB move with Life Orb boost secures the KO",
  "Opponent at low HP, priority move finishes the job",
  "Walling the opponent while recovering HP",
  "Setting up hazards to pressure opponent switches",
]

function generateBattle(id, botVersion) {
  const lead = POKEMON[Math.floor(Math.random() * POKEMON.length)]
  const oppLead = POKEMON[Math.floor(Math.random() * POKEMON.length)]
  const won = Math.random() > (botVersion === "v4" ? 0.12 : botVersion === "v3" ? 0.18 : 0.3)
  const turns = Math.floor(Math.random() * 40) + 8
  const remaining = won ? Math.floor(Math.random() * 4) + 1 : 0
  const move = MOVES[Math.floor(Math.random() * MOVES.length)]
  const reason = REASONS[Math.floor(Math.random() * REASONS.length)]
  const llm = Math.random() > 0.15
  return { id, won, turns, lead, oppLead, remaining, botVersion, move, reason, llm, time: new Date() }
}
    ...
    <div style={{
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <Pokeball size={32} />
          <span style={{ fontFamily: "'Press Start 2P', monospace", fontSize: "12px", color: C.yellow }}>
            SHOWDOWN AI
          </span>
        </div>
      ...
      <div style={{
        ...
        <div style={{ animation: "slideUp 0.8s ease forwards" }}>
          ...
          <p style={{ fontSize: "16px", color: C.muted, maxWidth: "500px", margin: "20px auto 40px", lineHeight: 1.7 }}>
            ...
            opponent set prediction, and smart heuristics. Built with poke-env.
          </p>

          {/* Stats */}
          <div style={{ display: "flex", gap: "30px", justifyContent: "center", marginBottom: "40px", flexWrap: "wrap" }}>
            {[
              { label: "SPRINTS", value: "4 / 6", color: C.red },
              { label: "ENGINE", value: "Claude", color: C.blue },
              { label: "FORMAT", value: "Gen 9", color: C.yellow },
            ].map((s, i) => (
              <div key={i} style={{
          ...
          <div style={{ marginTop: "60px", display: "flex", gap: "24px", flexWrap: "wrap", justifyContent: "center" }}>
            {["Python", "poke-env", "Claude API", "Opponent Modeling", "Next.js"].map((t, i) => (
              <span key={i} style={{
...
function Dashboard({ onBack }) {
  ...
  const [selectedBot, setSelectedBot] = useState("v4")
  ...
  const llmRate = battles.length > 0 ? ((battles.filter(b => b.llm).length / battles.length) * 100).toFixed(0) : 0
  ...
  const stopBattles = () => { clearInterval(intervalRef.current); setIsRunning(false); setCurrentBattle(null) }
  const resetStats = () => { setBattles([]); setCurrentBattle(null) }
      ...
      <div style={{
        ...
        {isRunning && (
          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
            ...
            <span style={{ fontFamily: "'Press Start 2P', monospace", fontSize: "8px", color: "#4CAF50" }}>BATTLING</span>
          </div>
        )}
      </div>
      ...
      <div style={{ padding: "24px", maxWidth: "1200px", margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
          ...
          <div style={{ background: C.card, borderRadius: "8px", padding: "20px", border: `1px solid ${C.cardLight}` }}>
            <h3 style={{ fontFamily: "'Press Start 2P', monospace", fontSize: "11px", color: C.yellow, margin: "0 0 20px 0" }}>
              🎮 BATTLE CONTROL
            </h3>

            <div style={{ marginBottom: "16px" }}>
              <label style={{ fontFamily: "'Press Start 2P', monospace", fontSize: "8px", color: C.muted, display: "block", marginBottom: "8px" }}>SELECT BOT</label>
              <div style={{ display: "flex", gap: "8px" }}>
                {[
                  { id: "v2", label: "Sprint 2", sub: "Heuristic" },
                  { id: "v3", label: "Sprint 3", sub: "LLM" },
                  { id: "v4", label: "Sprint 4", sub: "LLM+Predict" },
                ].map(bot => (
                  <button key={bot.id} onClick={() => setSelectedBot(bot.id)} style={{
                    flex: 1, background: selectedBot === bot.id ? C.red + "33" : C.darker,
                    border: `2px solid ${selectedBot === bot.id ? C.red : C.cardLight}`,
                    borderRadius: "6px", padding: "10px", cursor: "pointer", color: C.text,
                  }}>
                    <div style={{ fontFamily: "'Press Start 2P', monospace", fontSize: "8px", marginBottom: "4px" }}>{bot.label}</div>
                    <div style={{ fontSize: "11px", color: C.muted }}>{bot.sub}</div>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div style={{ marginBottom: "16px" }}>
              <label style={{ fontFamily: "'Press Start 2P', monospace", fontSize: "8px", color: C.muted, display: "block", marginBottom: "8px" }}>BATTLES</label>
              <div style={{ display: "flex", gap: "8px" }}>
                {[1, 10, 25, 50].map(n => (
                  <button key={n} onClick={() => setBattleCount(n)} style={{
                    flex: 1, background: battleCount === n ? C.blue + "33" : C.darker,
                    border: `2px solid ${battleCount === n ? C.blue : C.cardLight}`,
                    borderRadius: "6px", padding: "8px", cursor: "pointer",
                    color: battleCount === n ? C.blue : C.muted,
                    fontFamily: "'Press Start 2P', monospace", fontSize: "10px",
                  }}>{n}</button>
                ))}
              </div>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              <button onClick={isRunning ? stopBattles : startBattles} style={{
                flex: 1,
                background: isRunning ? `linear-gradient(135deg, ${C.red}, #cc2200)` : `linear-gradient(135deg, #4CAF50, #388E3C)`,
                border: "none", color: "white", padding: "12px", borderRadius: "6px",
                cursor: "pointer", fontFamily: "'Press Start 2P', monospace", fontSize: "10px",
              }}>
                {isRunning ? "⏹ STOP" : "▶ START"}
              </button>
              <button onClick={resetStats} style={{
                background: C.darker, border: `1px solid ${C.cardLight}`,
                color: C.muted, padding: "12px 16px", borderRadius: "6px",
                cursor: "pointer", fontFamily: "'Press Start 2P', monospace", fontSize: "9px",
              }}>↺</button>
            </div>
            ...
            {currentBattle && (
              <div style={{
                marginTop: "16px", background: C.darker, borderRadius: "6px",
                padding: "12px", border: `1px solid ${C.yellow}33`, animation: "slideIn 0.3s ease",
              }}>
                <div style={{ fontFamily: "'Press Start 2P', monospace", fontSize: "8px", color: C.yellow, marginBottom: "10px" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    ...
                    <div style={{ display: "flex", gap: "4px" }}>{currentBattle.lead.types.map(t => <TypeBadge key={t} type={t}/>)}</div>
                  </div>
                  <div style={{ fontFamily: "'Press Start 2P', monospace", fontSize: "10px", color: C.red }}>VS</div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: "14px", fontWeight: 700, marginBottom: "4px" }}>{currentBattle.oppLead.name}</div>
                    <div style={{ display: "flex", gap: "4px", justifyContent: "flex-end" }}>{currentBattle.oppLead.types.map(t => <TypeBadge key={t} type={t}/>)}</div>
                  </div>
                </div>
          ...
          <div style={{ background: C.card, borderRadius: "8px", padding: "20px", border: `1px solid ${C.cardLight}` }}>
            ...
            <div style={{ marginTop: "20px" }}>
              ...
              <div style={{ display: "flex", gap: "4px", flexWrap: "wrap" }}>
                ...
                {battles.length === 0 && <span style={{ color: C.muted, fontSize: "12px" }}>No battles yet</span>}
              </div>
            </div>

            {battles.length > 0 && (
              <div style={{ marginTop: "20px", padding: "12px", background: C.darker, borderRadius: "6px" }}>
                <div style={{ fontFamily: "'Press Start 2P', monospace", fontSize: "8px", color: C.muted, marginBottom: "8px" }}>BOT COMPARISON</div>
                {["v2", "v3", "v4"].map(v => {
                  const vB = battles.filter(b => b.botVersion === v)
                  const vW = vB.filter(b => b.won).length
                  const vWr = vB.length > 0 ? ((vW / vB.length) * 100).toFixed(0) : "—"
                  const label = v === "v2" ? "Heuristic" : v === "v3" ? "LLM" : "LLM+Predict"
                  return (
                    <div key={v} style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                      <span style={{ fontSize: "12px" }}>Sprint {v.slice(1)} ({label})</span>
                      <span style={{ fontFamily: "'Press Start 2P', monospace", fontSize: "10px", color: v === "v4" ? "#4CAF50" : C.muted }}>
                        {vWr}% ({vW}/{vB.length})
                      </span>
                    </div>
                  )
        ...
        <div style={{ marginTop: "24px", background: C.card, borderRadius: "8px", padding: "20px", border: `1px solid ${C.cardLight}` }}>
          <h3 style={{ fontFamily: "'Press Start 2P', monospace", fontSize: "11px", color: C.yellow, margin: "0 0 16px 0" }}>📜 BATTLE LOG</h3>
            ...
            {battles.length === 0 ? (
              ...
              battles.map((b, i) => (
                <div key={i} style={{
                  ...
                  <span style={{ fontFamily: "'Press Start 2P', monospace", fontSize: "8px", color: C.muted, minWidth: "30px" }}>#{b.id}</span>
                  <span style={{ fontFamily: "'Press Start 2P', monospace", fontSize: "9px", color: b.won ? "#4CAF50" : C.red, minWidth: "36px" }}>
                    {b.won ? "WIN" : "LOSS"}
                  </span>
                  <span style={{ fontSize: "13px", fontWeight: 600, minWidth: "100px" }}>{b.lead.name}</span>
                  <span style={{ fontSize: "11px", color: C.muted }}>vs</span>
                  <span style={{ fontSize: "13px", fontWeight: 600, minWidth: "100px" }}>{b.oppLead.name}</span>
                  <div style={{ display: "flex", gap: "4px" }}>{b.oppLead.types.map(t => <TypeBadge key={t} type={t}/>)}</div>
                  <span style={{
                    fontSize: "10px", padding: "2px 6px", borderRadius: "3px",
                    background: b.llm ? "#00BCD422" : C.red + "22",
                    color: b.llm ? "#00BCD4" : C.red,
                    border: `1px solid ${b.llm ? "#00BCD444" : C.red + "44"}`,
                  }}>
                    {b.llm ? "LLM" : "HEUR"}
                  </span>
                  <span style={{ marginLeft: "auto", fontSize: "11px", color: C.muted }}>{b.turns}t</span>
                  {b.won && <span style={{ fontSize: "11px", color: "#4CAF50" }}>{b.remaining}/6</span>}
                </div>
              ))
            )}
          </div>
